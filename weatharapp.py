...
def login_window():
    login_root = Tk()
    login_root.title("reg")
    login_root.title("Login")

    Label(login_root, text="Registration").pack(pady=5)
    ...
    def reg_user():
        # Check credentials here - replace the conditions with your own validation logic
        if username_entry.get() == "ashish" and password_entry.get() == "ashish":
            login_root.destroy()
            #show_weather_app()
        else:
            print("Invalid credentials")  # Handle invalid login

    login_button = Button(login_root, text="Register", command=reg_user)
    login_button.pack()

    Label(login_root, text="Login").pack(pady=5)
    username_label = Label(login_root, text="Username:")
    username_label.pack()
    username_entry = Entry(login_root)
    username_entry.pack()
    ...
    login_button = Button(login_root, text="Login", command=login_user)
    login_button.pack()
